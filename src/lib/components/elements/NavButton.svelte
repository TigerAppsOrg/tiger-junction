...
<a
    ...
    class="h-12 w-full justify-center items-center flex border-l-4">
    <div class="flex justify-start items-center gap-1 w-full">
        <img
            {src}
            {alt}
            class="{size} mr-1"
            class:bright={selected}
            class:dim={!selected} />
        <span class="font-light text-lg">{text}</span>
    </div>
    <!-- * Left -->
...
<style lang="postcss">
    ...
    .bright {
        @apply dark:invert;
        filter: invert(47%) sepia(91%) saturate(1892%) hue-rotate(207deg)
