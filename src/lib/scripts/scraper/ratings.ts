        ...
        if (data2 && data2.length !== 0) {
            ...
            for (let i = data2.length - 1; i >= 0; i--) {
                ...
                if (
                    ...
                    data3.length > 0 &&
                    data3[0].evaluations &&
                    (data3[0].evaluations as unknown as AppeaseTS).rating !==
                        null
                ) {
                    ...
                    if (checkInstructors) {
                        for (let j = 0; j < data3[0].instructors.length; j++) {
                            if (
                                currentInstructors.includes(
                                    data3[0].instructors[j].netid
                                )
                            ) {
                                await supabase
                                    .from("courses")
