...
for (let i = 0; i < courselist.length; i++) {
    ...
    const rawCourseData = await fetch(
        `${API_URL}${term}&course_id=${course.listing_id}`,
        {
            method: "GET",
            headers: {
                Authorization: token
    ...
    const courseData = (await rawCourseData.json()).course_details
        .course_detail[0];
...
console.log(
    `Average response time: ${responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length}ms`
);
const endTime = new Date().getTime();
console.log(
    `Wrote ${cacheData.length} courses to ${filename} in ${(endTime - startTime) / 1000}s.`
);
