...
async function snatchFetch(
  ...
  if (!config)
    return {
      ok: false,
      error: "TigerSnatch is not configured (missing SNATCH_URL or SNATCH_TOKEN).",
    };
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ok: false,
        error: data?.message ?? `TigerSnatch returned status ${response.status}`,
      };
    }
    return { ok: true, data };
  } catch (err) {
    return {
      ok: false,
      error: `Failed to reach TigerSnatch: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
}
...
async function resolveClassId(
  ...
  if (courses.length === 0) {
    return {
      error: `Course "${courseCode}" not found${term ? ` for ${termCodeToName(term)}` : ""}.`,
    };
  }
  ...
  if (sections.length === 0) {
    return {
      error: `No sections found for ${course.code}${sectionTitle ? ` section ${sectionTitle}` : ""}.`,
    };
  }
...
export function registerSnatchTools(
  ...
  server.registerTool(
    ...
    async () => {
      if (!authContext?.netid) {
        return {
          content: [
            { type: "text" as const, text: "Requires authenticated user (x-user-netid header)." },
          ],
          isError: true,
        };
      }
      ...
      if (!result.ok) {
        return {
          content: [
            { type: "text" as const, text: result.error ?? "Failed to get subscriptions." },
          ],
          isError: true,
        };
      }
          ...
          content: [
            {
              ...
              text: JSON.stringify(
                {
                  subscriptions: [],
                  message: "No TigerSnatch account found. Visit tigersnatch.com to get started.",
                },
                null,
                2
              ),
  ...
  server.registerTool(
    "subscribe_to_snatch",
    {
      ...
      inputSchema: z.object({
        ...
        section: z
          ...
          .optional()
          .describe(
            "Section title (e.g., 'L01', 'P01'). If omitted and the course has multiple sections, available sections will be listed."
          ),
        term: z
          .number()
          .optional()
          .describe("Term code (e.g., 1272 for Fall 2026). Defaults to most recent term."),
      }),
    },
    async ({ courseCode, section, term }) => {
      ...
      const result = await snatchFetch(
        `/junction/add_to_waitlist/${authContext.netid}/${resolved.classId}`
      );
      if (!result.ok) {
        return {
          content: [{ type: "text" as const, text: result.error ?? "Failed to subscribe." }],
          isError: true,
        };
      }
  ...
  server.registerTool(
    "unsubscribe_from_snatch",
    {
      description: "Unsubscribe from TigerSnatch notifications for a class section.",
      inputSchema: z.object({
        courseCode: z.string().describe("Course code (e.g., 'COS 226')."),
    ...
    async ({ courseCode, section, term }) => {
      ...
      const result = await snatchFetch(
        `/junction/remove_from_waitlist/${authContext.netid}/${resolved.classId}`
      );
      if (!result.ok) {
        return {
          content: [{ type: "text" as const, text: result.error ?? "Failed to unsubscribe." }],
          isError: true,
        };
      }
  ...
  server.registerTool(
    ...
    async ({ courseCode }) => {
      ...
      const course = await snatchDb.collection("mappings").findOne({
        displayname: {
          $regex: new RegExp(`^${normalizedCode.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`, "i"),
        },
      });
      ...
      if (!course) {
        ...
        const wsMatch = await snatchDb.collection("mappings").findOne({
          displayname_whitespace: {
            $regex: new RegExp(courseCode.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"),
          },
        });
        if (!wsMatch) {
          return {
            content: [
              { type: "text" as const, text: `Course "${courseCode}" not found in TigerSnatch.` },
            ],
            isError: true,
          };
        }
        return await buildDemandResponse(
          snatchDb,
          wsMatch.courseid,
          wsMatch.displayname_whitespace,
          wsMatch.title
        );
      }
  ...
  server.registerTool(
    ...
    async ({ limit: maxResults }) => {
      ...
      if (!admin) {
        return {
          content: [{ type: "text" as const, text: "TigerSnatch admin data not available." }],
          isError: true,
        };
      }
      ...
      const topEnrolled = await snatchDb
  ...
  server.registerTool(
    ...
    async ({ courseCode }) => {
      ...
      if (courseRows.length === 0) {
        return {
          content: [{ type: "text" as const, text: `Course "${courseCode}" not found.` }],
          isError: true,
        };
      }
      ...
      const avgFillRate =
        termsWithEnrollment > 0 ? Math.round(totalFillRateSum / termsWithEnrollment) : 0;

      // Determine competitiveness
      let competitiveness: string;
      if (avgFillRate >= 95 || termsFullyClosed >= termsWithEnrollment * 0.5) {
        competitiveness =
          "Very Competitive — this course consistently fills up and often closes. Plan to enroll early or use TigerSnatch.";
      } else if (avgFillRate >= 80) {
        competitiveness =
          "Competitive — this course typically fills most of its seats. Early enrollment recommended.";
      } else if (avgFillRate >= 50) {
        competitiveness =
          "Moderate — this course usually has available seats but does fill up in popular sections.";
      } else {
        competitiveness = "Low — this course generally has plenty of available seats.";
      }

      // Capacity trend
      const capsOverTime = termStats.filter((t) => t.totalCapacity > 0).map((t) => t.totalCapacity);
      ...
      if (capsOverTime.length >= 2) {
        ...
        if (change > 15)
          capacityTrend = `Growing (+${Math.round(change)}% capacity since ${termStats[0].termName})`;
        else if (change < -15)
          capacityTrend = `Shrinking (${Math.round(change)}% capacity since ${termStats[0].termName})`;
      }
...
async function buildDemandResponse(
  snatchDb: Db,
  courseid: string,
  displayName: string,
  title: string
  ...
  const enrollments = await snatchDb.collection("enrollments").find({ courseid }).toArray();
  ...
  for (const e of enrollments) {
    ...
    const waitlistDoc = await snatchDb
      .collection("waitlists")
      .findOne({ classid: String(e.classid) });
        ...
        text: JSON.stringify(
          {
            ...
            overallFill:
              totalCapacity > 0
                ? `${totalEnrollment}/${totalCapacity} (${Math.round((totalEnrollment / totalCapacity) * 100)}%)`
                : "N/A",
            totalSubscribers,
